...
%A Network-CDNÊdiffers from a traditional ISP in that it controls both the \emph{content placement} and routing in the network while a non-CDN ISP only controls routing. A Network CDN can have traffic due to content for which it cannot control content placement but we do not consider that as a part of our discussion. 
