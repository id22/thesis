...
Given the huge body of prior work specifically on mobility as well as broader efforts on Internet architecture, it is natural to ask:  is a global name resolution service critical to handling mobility if we had the luxury of re-architecting Internet naming and routing from a clean slate? %In this section, we first present subjective arguments to justify an affirmative position on this question, and then describe DNS's shortcomings as a resolution service under high mobility for today's Internet.
